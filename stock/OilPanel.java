...
public class OilPanel extends JPanel {
    ...
    public OilPanel() {
        ...
        setBorder(BorderFactory.createTitledBorder("기름 시세 정보"));
        ...
        // 휘발유 패널
        JPanel usdPanel = createInfoPanel("휘발유 시세", Color.BLUE);
        usdKrwRateLabel = (JLabel) usdPanel.getComponent(0);

        // 고급휘발유 패널
        JPanel jpyPanel = createInfoPanel("고급휘발유 시세", new Color(0, 128, 0));
    ...
    private void fetchExchangeRates() {
        SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>() {
            ...
            @Override
            protected void done() {
                usdKrwRateLabel.setText("1 리터 = " + usdRate + " KRW");
                jpyKrwRateLabel.setText("1 리터 = " + jpyRate + " KRW");
    ...
    private String fetchExchangeRate(String currency) {
        try {
    		...
    		System.out.println("휘발유시세: "+header.get(3).select("tbody").text().split(" ")[9]);
    		System.out.println("고급휘발유시세: "+header.get(3).select("tbody").text().split(" ")[13]);
    		//System.out.println(header.get(0).select("tbody").text());
    		String dollor=header.get(3).select("tbody").text().split(" ")[9];
    ...
    private String fetchExchangeRate2(String currency) {
        try {
    		...
    		String enn=header.get(3).select("tbody").text().split(" ")[13];
    ...
    public static void showInFrame() throws ClassNotFoundException, InstantiationException, IllegalAccessException, UnsupportedLookAndFeelException {
        JFrame frame = new JFrame("기름 가격");
        frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
        frame.getContentPane().add(new OilPanel());
        frame.pack();
        frame.setLocation(980,260);
